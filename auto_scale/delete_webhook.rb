...
Examples:
  #{__FILE__} --group=my_group --policy=my_policy
...
abort "\nThere are not any scaling groups in the Chicago region. Try running create_scaling_group.rb\n\n" if groups.empty?
webhook = webhooks.find {|wh| wh.name == opts['--webhook']}
abort "\nPlease select a webhook. Available webhook: #{webhooks.map{ |wh| wh.name }}" if webhook.nil?
